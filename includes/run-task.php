 ...
 *
 * 由 class-bokeauto.php 的 api_chat 以独立 OS 进程拉起（Windows: start /B；Linux: exec &）。
...
// 定位 WordPress 根目录的 wp-load.php（本文件位于 wp-content/plugins/bokeauto/includes/）
...
if ( ! class_exists( 'Bokeauto_Agent' ) ) {
	fwrite( STDERR, "Bokeauto_Agent not loaded\n" );
	exit( 1 );
}
...
if ( ! empty( $meta['role_id'] ) ) {
	$agent->role = Bokeauto_Role::get( (int) $meta['role_id'] );
}
...
try {
	...
	if ( $conv_id && isset( $done['text'] ) && '' !== trim( (string) $done['text'] ) ) {
		Bokeauto_Conversation::add_message( $conv_id, 'assistant', $done['text'] );
	}
	if ( isset( $done['usage'] ) && is_array( $done['usage'] ) ) {
		Bokeauto_Usage::log( (int) $meta['user_id'], $conv_id, $done['usage']['prompt_tokens'], $done['usage']['completion_tokens'] );
	}
